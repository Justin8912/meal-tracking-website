...
### 2026-06-01 — production-correctness fix (post-bundle)
- The `/plans/summary` route imports `@meal-tracking/nutrition-engine` at runtime, but the engine was only an `apps/api` devDependency and was never copied/built/shipped by `apps/api/Dockerfile` — `npm prune --omit=dev` would drop it, so the compiled production image would crash with MODULE_NOT_FOUND on `/api/v1/plans/summary`. Per-workspace tsc/vite did not catch this (workspace resolution ignores dep classification).
- Fix (packaging only, no engine/route logic changes): promoted the engine from devDependencies to dependencies in `apps/api/package.json` (mirroring `@meal-tracking/shared`); added `npm run build -w @meal-tracking/nutrition-engine` to the api `build` script (after shared, before the api tsc); added the engine's build-stage and runtime-stage COPY lines to `apps/api/Dockerfile` (mirroring shared) and updated the header comment.
- Verified: all four workspaces typecheck clean; `npm run build -w @meal-tracking/api` emits both `packages/nutrition-engine/dist/index.js` and `apps/api/dist/...`; the engine resolves to compiled `dist/index.js` (not TS source) under `NODE_ENV=production`. Docker image build is blocked by the corporate TLS/Artifactory auth proxy (`npm ci` E401 inside the build, even with a gitignored `*.verify` CA overlay), so the prune was simulated instead: a real offline `npm prune --omit=dev` against the installed tree KEEPS the engine (and all runtime deps) while dropping dev tooling (tsx/vitest/typescript), and the engine resolves the compiled dist from that pruned tree. Full DB-backed api suite green on re-run (the documented ECONNREFUSED flake recurred once, passed on re-run).
